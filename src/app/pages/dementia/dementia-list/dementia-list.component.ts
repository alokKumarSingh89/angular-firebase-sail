...
@Component({
  selector: 'dementia-list',
  templateUrl: 'dementia-list.component.html',
  styleUrls: ['./dementia-list.scss']
})
export class DementiaListComponent {
  name: string;
  ...
  public constructor(private _pagesService: PagesService, private _modalService: NgbModal, private _toasterService: ToasterService, protected router: Router, private _globals: Globals, private loaderService: LoaderService, public orderPipe: OrderPipe) {
    ...
    this.canCreate = _globals.canCreate('Dementia');
    this.canEdit = _globals.canEdit('Dementia');
    this.canView = _globals.canView('Dementia');
  ...
  public getUpdatesList() {
    this._pagesService.getNodes('Dementia').subscribe(
      data => {
        this.tableData = [];
        let tempData: any = {};
        for (var key in data) {
  ...
  public onCellEdit(data: any) {
    this.router.navigate(['/pages/dementia/edit/' + data.id])
  }
    ...
    this.activeModal.result.then((results) => {
      ...
      if (results != undefined && results != 'yes') {
        ...
        this._pagesService.updateNode(postObj).subscribe(
          data => {
            this.title = "Dementia Update";
            this.content = "Dementia updated successfully."
            this.showToast(this.type, this.title, this.content);
            this.getUpdatesList();
          },
          error => {
